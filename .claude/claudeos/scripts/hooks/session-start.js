...
function calcWeekPhase(startDate) {
  ...
  const weeks =
    Math.floor((Date.now() - start) / (7 * 24 * 60 * 60 * 1000)) + 1;
  if (weeks <= 8)
    return {
      week: weeks,
      phase: "Build",
      focus: "実装優先 / Agent Teams パターン A",
    };
  if (weeks <= 16)
    return {
      week: weeks,
      phase: "Quality",
      focus: "テスト・レビュー強化 / パターン B",
    };
  if (weeks <= 20)
    return {
      week: weeks,
      phase: "Stabilize",
      focus: "新機能凍結 / CI 安定化のみ",
    };
  return {
    week: weeks,
    phase: "Release",
    focus: "変更最小化 / セキュリティ最終確認",
  };
}
    ...
    process.stdout.write(
      JSON.stringify({
        hookSpecificOutput: {
          ...
          additionalContext:
            "[SessionStart] state.json not found — fresh session\n" +
            ICON_DIRECTIVE,
        },
      }),
    );
...
lines.push(
  `  last_pre_compact_at: ${compact.last_pre_compact_at || "(never)"}`,
);
...
if (blocked.length > 0) {
  lines.push(
    `  blocked_issues: ${blocked.map((b) => (typeof b === "object" ? b.issue || b : b)).join(", ")}`,
  );
}
...
if (rot && rot.mode === "phase") {
  ...
  const forced = (state.warnings || [])
    .filter((w) => w.kind === "goal_rotation_forced_advance")
    .slice(-1)[0];
  if (forced) {
    lines.push(
      `  goal_rotation_warning: ⚠️ 前回 ${forced.phase || "?"} フェーズが未達のまま強制前進 — 未達項目を本セッションで考慮すること`,
    );
  }
...
// v10.7: 直近の Session Handoff Summary はポインタのみ注入する。
// SessionStart は compact 再開ごとにも発火するため、本文抜粋(≤1500字)の注入は
// 毎回の固定 context コストとなり autocompact スラッシングの一因になる。
// 内容が必要なときだけ Read で参照させる。
try {
  ...
  if (fs.existsSync(handoffDir)) {
    ...
    if (latest) {
      lines.push("");
      lines.push(
        `[Session Handoff] 直近の引き継ぎ: reports/handoff/${latest} (内容が必要な場合のみ Read で参照)`,
      );
    }
  }
} catch {
...
function recommendPattern(phase) {
  ...
  if (p.includes("monitor") || p.includes("research") || p.includes("design")) {
    return {
      pattern: "C",
      desc: "調査・設計 (技術調査 + 設計 + Devil's Advocate)",
    };
  }
  return null;
}
...
if (rec) {
  lines.push(
    `  agent_teams_recommended: パターン ${rec.pattern} — ${rec.desc}`,
  );
}
...
if (atuCur.team_create_count || atuCur.send_message_count) {
  lines.push(
    `  agent_teams_current: TeamCreate=${atuCur.team_create_count || 0} SendMessage=${atuCur.send_message_count || 0} patterns=[${(atuCur.patterns_used || []).join(",")}]`,
  );
}
...
lines.push(
  `  dashboard: http://localhost:${dashPort}/mission-control (Agent Teams Activity パネル参照)`,
);
...
try {
  ...
  const patterns =
    typeof rb.retrieveRelevantPatternsGlobal === "function"
      ? rb.retrieveRelevantPatternsGlobal(
          bank,
          projectName,
          phase,
          currentTags,
  ...
  if (patterns.length > 0) {
    ...
    patterns.forEach((p, i) => {
      ...
      lines.push(
        `  [${i + 1}] conf=${confStr} | ${p.outcome} | phase=${p.phase} | tags=[${tagsStr}]${crossMk}`,
      );
      lines.push(`       問題: ${p.problem_pattern}`);
      const approachPreview = (p.approach || "").slice(0, 120);
      lines.push(
        `       対応: ${approachPreview}${(p.approach || "").length > 120 ? "…" : ""}`,
      );
    });
  }
} catch (_rbErr) {
