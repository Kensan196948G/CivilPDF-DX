...
v1.2.0 (Tauri v2, unsigned stable, text edit mode + Phase A/B/C features included).
"""

# Real asset filenames as attached to GitHub Release v1.2.0. Filenames contain
# "1.2.0" matching the version set in package.json / tauri.conf.json for this
# release. GitHub replaces spaces with dots in asset names.
# Download URLs are `{base}/{filename}`.
_BASE = "https://github.com/Kensan196948G/CivilPDF-Editor/releases/download/v1.2.0"
_REAL_FILENAMES = {
    "win-exe": "CivilPDF.Editor_1.2.0_x64-setup.exe",
    "win-msi": "CivilPDF.Editor_1.2.0_x64_en-US.msi",
    "mac-dmg": "CivilPDF.Editor_1.2.0_universal.dmg",
    "linux-deb": "CivilPDF.Editor_1.2.0_amd64.deb",
    "linux-appimage": "CivilPDF.Editor_1.2.0_amd64.AppImage",
    "linux-rpm": "CivilPDF.Editor-1.2.0-1.x86_64.rpm",
}
...
class TestReleases:
    def test_releases_structure(self, client, admin_token):
        ...
        data = resp.json()
        assert data["stable_version"] == "v1.2.0"
        assert isinstance(data["packages"], list)
        assert isinstance(data["channels"], list)
        # Only the stable channel exists for v1.2.0.
        assert len(data["channels"]) == 1
        assert data["channels"][0]["id"] == "stable"
        assert data["channels"][0]["version"] == "v1.2.0"
    ...
    def test_real_filenames_and_version(self, client, admin_token):
        ...
        for pkg_id, filename in _REAL_FILENAMES.items():
            ...
            # version field reports the release version (1.2.0), not the filename fragment.
            assert by_id[pkg_id]["version"] == "1.2.0"
...
class TestReleaseNotes:
    ...
    def test_notes_describe_real_features(self, client, admin_token):
        ...
        # v1.2.0 headline feature: text edit mode.
...
class TestBuildInfo:
    ...
    def test_build_info_env_injection(self, client, admin_token, monkeypatch):
        ...
        monkeypatch.setenv("APPS_BUILD_NUMBER", "1.2.0+build.42")
        resp = client.get("/api/v1/apps/build-info", headers=_auth(admin_token))
        assert resp.status_code == 200
        data = resp.json()
        assert data["git_commit"] == "abc1234"
        assert data["build_date"] == "2026-06-22"
        assert data["build_number"] == "1.2.0+build.42"
