...
## 3. Neon 利用仕様（ワークスペース共通・本リポジトリは対象外）

> **注意**: CivilPDF-DX は 2026-09-18 に Neon を廃止した（本番DBはホスト上のローカル PostgreSQL。
> docs/deployment/local-postgresql.md 参照）。本章はワークスペース共通の記録として保持するが、
> 本リポジトリの運用には適用されない。
