...
Source of truth: the public GitHub Release of CivilPDF-Editor (Tauri v2 desktop app).
    ...
    https://github.com/Kensan196948G/CivilPDF-Editor/releases/tag/v1.2.1
...
# Current published stable release. Matches the GitHub Release tag v1.2.1.
_VERSION = "1.2.1"
# Public release date of v1.2.1 (GitHub Release publication date).
...
def _base_url() -> str:
    ...
    Set in deployment to:
        https://github.com/Kensan196948G/CivilPDF-Editor/releases/download/v1.2.1
    """
...
def _build_packages() -> list[ReleasePackage]:
    ...
    Filenames MUST match the real assets attached to the GitHub Release v1.2.1
    (GitHub replaces spaces in asset names with dots). Asset filenames contain
    "1.2.1" because package.json / tauri.conf.json version was correctly bumped
    to 1.2.1 before the CI build. The download URL is then
    ...
    return [
        _pkg(
            ...
            "exe",
            "インストーラー (.exe / NSIS)",
            "CivilPDF.Editor_1.2.1_x64-setup.exe",
            "約 1.9 MB",
        ),
        _pkg(
            "win-msi",
            ...
            "msi",
            "インストーラー (.msi)",
            "CivilPDF.Editor_1.2.1_x64_en-US.msi",
            "約 2.4 MB",
        ),
        _pkg(
            "mac-dmg",
            ...
            "CivilPDF.Editor_1.2.1_universal.dmg",
            "約 4.5 MB",
        ),
        _pkg(
            "linux-deb",
            ...
            "deb",
            "Debian / Ubuntu (.deb)",
            "CivilPDF.Editor_1.2.1_amd64.deb",
            "約 2.3 MB",
        ),
        _pkg(
            "linux-appimage",
        ...
        _pkg(
            ...
            "rpm",
            "Fedora / RHEL (.rpm)",
            "CivilPDF.Editor-1.2.1-1.x86_64.rpm",
            "約 2.3 MB",
        ),
    ]
...
_CHANNELS: list[ChannelInfo] = [
    ChannelInfo(
        ...
        description=(
            "安定版。テキスト編集モード（v1.2.1 新機能）・注釈（Phase A）・"
...
_RELEASE_NOTES: list[ReleaseNote] = [
    ReleaseNote(
        ...
        channel="stable",
        release_date=_RELEASE_DATE,
        summary="v1.2.1 安定版 — テキスト編集モードの日本語対応を修正（v1.2.0 の全機能を継続搭載）",
        items=[
            ReleaseNoteItem(
                type="FIX",
                text="テキスト編集モードの日本語対応を修正（NotoSansJP 同梱で日本語が PDF に焼き込まれるように）",
            ),
            ReleaseNoteItem(
                type="FEAT",
        ...
        highlights=(
            ...
            "修正（v1.2.1）:\n"
            "- テキスト編集モードの日本語対応: NotoSansJP を同梱し、日本語の置換テキストが"
            "PDF に正しく焼き込まれるよう修正（従来は白塗りのみで反映されなかった）\n\n"
            "v1.2.0 の新機能:\n"
