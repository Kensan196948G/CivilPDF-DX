...
Source of truth: the public GitHub Release of CivilPDF-Editor (Tauri v2 desktop app).
    ...
    https://github.com/Kensan196948G/CivilPDF-Editor/releases/tag/v1.2.2
...
# Current published stable release. Matches the GitHub Release tag v1.2.2.
_VERSION = "1.2.2"
# Public release date of v1.2.2 (GitHub Release publication date).
...
def _base_url() -> str:
    ...
    Set in deployment to:
        https://github.com/Kensan196948G/CivilPDF-Editor/releases/download/v1.2.2
    """
...
def _build_packages() -> list[ReleasePackage]:
    ...
    Filenames MUST match the real assets attached to the GitHub Release v1.2.2
    (GitHub replaces spaces in asset names with dots). Asset filenames contain
    "1.2.2" because package.json / tauri.conf.json version was correctly bumped
    to 1.2.2 before the CI build. The download URL is then
    ...
    return [
        _pkg(
            ...
            "exe",
            "インストーラー (.exe / NSIS)",
            "CivilPDF.Editor_1.2.2_x64-setup.exe",
            "約 1.9 MB",
        ),
        _pkg(
            "win-msi",
            ...
            "msi",
            "インストーラー (.msi)",
            "CivilPDF.Editor_1.2.2_x64_en-US.msi",
            "約 2.4 MB",
        ),
        _pkg(
            "mac-dmg",
            ...
            "CivilPDF.Editor_1.2.2_universal.dmg",
            "約 4.5 MB",
        ),
        _pkg(
            "linux-deb",
            ...
            "deb",
            "Debian / Ubuntu (.deb)",
            "CivilPDF.Editor_1.2.2_amd64.deb",
            "約 2.3 MB",
        ),
        _pkg(
            "linux-appimage",
            ...
            "appimage",
            "AppImage (.AppImage)",
            "CivilPDF.Editor_1.2.2_amd64.AppImage",
            "約 80 MB",
        ),
        _pkg(
            "linux-rpm",
            ...
            "rpm",
            "Fedora / RHEL (.rpm)",
            "CivilPDF.Editor-1.2.2-1.x86_64.rpm",
            "約 2.3 MB",
        ),
    ]
...
_CHANNELS: list[ChannelInfo] = [
    ChannelInfo(
        ...
        description=(
            "安定版。テキスト編集モード（v1.2.2 新機能）・注釈（Phase A）・"
...
_RELEASE_NOTES: list[ReleaseNote] = [
    ReleaseNote(
        ...
        channel="stable",
        release_date=_RELEASE_DATE,
        summary="v1.2.2 安定版 — テキスト編集の画面反映を修正（v1.2.0 の全機能を継続搭載）",
        items=[
            ReleaseNoteItem(
                type="FIX",
        ...
        highlights=(
            ...
            "修正（v1.2.2）:\n"
            "- テキスト編集の画面反映: 確定した編集が画面に即時表示されるよう修正"
            "（従来は確定しても画面に変化が出なかった）\n"
            "- テキスト編集の日本語対応: NotoSansJP を同梱し、日本語の置換テキストが"
            "PDF に正しく焼き込まれるよう修正（v1.2.1）\n\n"
            "v1.2.0 の新機能:\n"
