...
// Static card definitions (stat.value is the fallback / infra-only static value)
const SECURITY_CARDS: SecurityCard[] = [
  ...
  },
  {
    ...
    stat: { label: "暗号化済みドキュメント", value: "—" },
    modalBody: `暗号化設定詳細\n\n転送暗号化: TLS 1.3（必須）\n保存暗号化: AES-256-GCM\nキー管理: Azure Key Vault\nPDF暗号化: 128bit RC4 / AES-256\n\n未暗号化ドキュメント: 0（全件保存時暗号化）\n証明書有効期限: 2026-03-15`,
  },
  {
    id: "watermark",
    title: "透かし",
  ...
  },
  {
    ...
    stat: { label: "総イベント数", value: "—" },
    modalBody: `監査ログ設定詳細\n\n記録方式: 追記専用（DELETE不可）\n改ざん防止: SHA-256 ハッシュチェーン\n保存期間: 7年（法的要件）\n\n記録イベント種別:\n- 認証（ログイン/ログアウト/失敗）\n- ドキュメント操作（閲覧/DL/印刷/編集）\n- 承認フロー（承認/否決/差戻し）\n- 管理操作（設定変更/ユーザー管理）\n\nSIEM連携: Splunk（設定済み）`,
  },
];

// Return live stat value for cards backed by real API; fall back to static value on load/error
function resolveStatValue(
  ...
  switch (id) {
    case "sso":
      return stats ? stats.active_users.toLocaleString() : staticValue;
    case "enc":
      return stats ? stats.total_documents.toLocaleString() : staticValue;
    case "auditlog":
      return auditTotal > 0 ? auditTotal.toLocaleString() : staticValue;
    default:
      return staticValue;
  }
}
...
export const SecurityView: FC<ViewProps> = ({ onShowModal }) => {
  ...
  useEffect(() => {
    Promise.all([getStats(), listAuditLogs({ per_page: 1 })])
      .then(([s, al]) => {
        setStats(s);
        setAuditTotal(al.total);
      })
      .catch(() => {
